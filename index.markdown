...
There have been a few successful non-image applications, to proteins, audio, text, and MRIs. There is an opinion that diffusion+related models are useful for solving conditional problems in the non-huge-data regime, or in the case when energy functions are known, or when there is no clear autoregressive ordering known for the data or when informative bits are not as uniformly spread across the data dimensions. These qualities make diffusion-like approaches relevant and promising for science applications: solving inverse problems in high dimensions, sampling problems in physics for large lattices, molecular docking, entropy computation, and more. 

If we'd like to continue applying diffusion models, how can we better understand what we trade off in the quality of solutions to these problems when we give up the well-studied-and-timeless Likelihood? And contrapositivly, if we explore closer connections between diffusion and likelihood, what can we gain?
