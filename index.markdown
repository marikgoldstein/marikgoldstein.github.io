...
There have been a few successful non-image diffusion applications, to proteins, audio, text. Common opnion is that diffusion+related models are useful for solving conditional problems in the non-huge-data regime---at scale where prompting doesn't arise---or when there is no clear autoregressive ordering known for the data. Other diffusion-favoring cases are when
