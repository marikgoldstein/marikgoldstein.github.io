...
Large transformers do well on discrete data tasks and are trained for likelihood on sequences of that data. Diffusions/flows are instead mostly studied in the context of natural images/video, and by contrast are not trained for likelihood. 

There have been a few successful non-image diffusion applications, to proteins, audio, text. Common opinion is that diffusion+related models are useful for solving conditional problems in the non-huge-data regime---at scale where prompting doesn't arise---or when there is no clear autoregressive ordering known for the data. Other diffusion-favoring cases are when
energy functions are known or when informative bits are not as uniformly spread across the data dimensions. These qualities make diffusion-like approaches relevant and promising for science applications: solving inverse problems in high dimensions, sampling problems in physics on large lattices, molecular docking under specific conditions, entropy computation of time-varying systems, accelerating MRI and more. 

If we'd like to continue applying diffusion models to such problems, how can we better understand what we trade off in the quality of solutions to these problems when we give up the well-studied-and-timeless Likelihood? Contrapositivly, if we explore closer connections between diffusion and likelihood, what can we gain in solving these problems?

More generally, how can we proceed when most non-autoregressive generative models have been developed super-specifically for chasing CIFAR and Imagenet performance metrics like FIDs? For example Latent Diffusion Models reduce dimensionality by only preserving the parts of the data that affect the activations of a specific image classifier (VGG16). For non-image problems, what do we use in place of the natural image classifier to train the encoder/decoder? There are no "labels" in general that preserve the important bits of information that we might want to keep (e.g. high-frequency parts of Navier-Stokes solutions), so what's the right bias for lower-dimensional-diffusion-friendly representations of high dimensional non-image problems?

On the health side, I think that conditional generative models, under the right assumptions, could help alleviate certain challenges in ML for the EHR, but currently, we may not be using these models in the right way. For example, it would be good to model the true distribution of a person's outcome given whichever features are available, without accidentally also conditioning on the implicit (likely non-transportable) feature missingness pattern itself. Doing so properly might require a mix of expertise and could benefit from collaboration, for example across generative models and causal inference. On transportability, how can we better assess transportability and clinical relevance of the models we build when it's so hard to move around models built in hospitals?
